<template>
	...
	const handleSetEq = (mode) => {
		...
		uni.showToast({ title: `已切换音效: ${['标准', '澎湃', '静谧'][mode]}`, icon: 'none' })
	}

	const handleQueryFiles = () => {
		uni.showToast({ title: '查询文件列表', icon: 'none' })
	}
	...
	const handleQueryBt = () => {
		uni.showToast({ title: '查询杰理版本', icon: 'none' })
	}

	const handleQueryLinux = () => {
		uni.showToast({ title: '查询富瀚版本', icon: 'none' })
	}

	const handleQueryGx8002 = () => {
		uni.showToast({ title: '查询国新版本', icon: 'none' })
	}

	const handleSend = () => {
		if (!sendData.value.trim()) {
			uni.showToast({ title: '请输入发送数据', icon: 'none' })
			return
		}
		uni.showToast({ title: '数据已发送', icon: 'success' })
	}

	const handleTakePhoto = () => {
		uni.showToast({ title: '拍照', icon: 'none' })
	}
	...
	const handleStopRecording = () => {
		uni.showToast({ title: '停止录像', icon: 'none' })
	}

	const handleSetDuration = (duration) => {
		uni.showToast({ title: `录制时长: ${['未设置', '15秒', '1分钟', '3分钟', '5分钟', '10分钟'][duration]}`, icon: 'none' })
	}
	</script>
